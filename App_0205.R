...
listCreate <- function(Freq,q,y,rmv) {
  Freq <- if (rmv == TRUE) {Freq %>%
    mutate(weight = Freq.y*(q+0.1)+Freq.x.x*2.01+Freq.y.y) %>% 
    mutate(weight = ifelse(Dino %in% excl, weight*1.1 , weight))  %>% filter(weight > 4.1)
  } else {Freq %>%
      mutate(weight = Freq.y*(q+0.1)+Freq.x.x*2.01+Freq.y.y) %>% 
      mutate(weight = ifelse(Dino %in% excl, weight*1.1 , weight))
  }
    
  if (rmv == TRUE) {Freq <<- Freq %>% filter(weight > 4.1)}
  ...
  fluidPage(theme = shinytheme("cyborg"),
            fluidRow(column(2,
                            ...
                            sidebarPanel(h3("Notes:"),br(),HTML("<ul><li><b>[Starting Date]:</b> Choose the first day of current round of poll to exclude old votes</li>
                                                                       ...
                                                                       <li><b>[Weight]:</b> 1st.Choices*(4+0.1) + 2ed.Choices*(2+0.01) + 3rd.Choices*1  </li>
                                                                       <li><b>[Lists Creating]:</b> 1. rank dinos by weight 2. decide number of dinos not in the column 1st.Choices from [Number of Dinos with 0 1st.Choices]
                                                                        3. every dino in column 1st.Choice be at least singled 4. double to fill the rest of slots</li></ul>"),width=15)
            ),              
            column(10,
