...
data1 <- read.csv(url,stringsAsFactors=FALSE,na.strings=c("","NA"))[1:6]

data1$Your.JWA.name <- str_trim(data1$Your.JWA.name, side ="right")
data1$Timestamp <- as.POSIXct(data1$Timestamp, format = "%m/%d/%Y %H:%M:%S")

Alliances <- c(data1$Alliance %>% unique())[-1]
...
listCreate <- function(Freq,q,w,rmv) {
  ...
  ##new method##
  ListA <- resultWeight2[c(1:2,3:10,seq(11,25,by=2),33:38),] 
  ListB <- resultWeight2[c(1:2,3:10,seq(12,26,by=2),39:44),]
  ListC <- resultWeight2[c(1:2,11:26,27:32),]
  
  names(ListA) <- c('List2','Weight','Status')
  names(ListB) <- c('List2','Weight','Status')
  names(ListC) <- c('List2','Weight','Status')
  
  ListA$Status <- c(rep('triple',2),rep('double',16),rep('single',6))
  ListB$Status <- c(rep('triple',2),rep('double',16),rep('single',6))
  ListC$Status <- c(rep('triple',2),rep('double',16),rep('single',6))
  
  rownames(ListA) <- c(1:24)
  rownames(ListB) <- c(1:24)
  rownames(ListC) <- c(1:24)
  ...
  observe({
    output$Each  <- renderPlot({
      ...
      if(input$radio == 1) {
        ...
        return(ggarrange(rank1Each[[1]],rank1Each[[2]],rank1Each[[3]],rank1Each[[4]],rank1Each[[5]],rank1Each[[6]],
                         nrow = 1))
      } else
      {return(NULL)}
    })
